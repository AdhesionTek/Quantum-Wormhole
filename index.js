...
function generateOnClick() {
  if (!nameInput.value || !birthInput.value) {
    ...
    paintCanvas(seed);
    if (
      nameInput.value.toLowerCase() === "ybb" ||
      nameInput.value.includes("海子")
    ) {
    ...
    if (
      nameInput.value.includes("嘉心糖") ||
      nameInput.value.includes("嘉然")
    ) {
    ...
    if (
      nameInput.value.toLowerCase().includes("ceylan") ||
      nameInput.value.includes("锡兰")
    ) {
